...
def train(opt, netG):
    if opt.vae_levels < opt.scale_idx + 1:
        D_curr = getattr(networks_new, opt.discriminator)(opt).to(opt.device)
    ...
    for iteration in epoch_iterator:
        ...
        generated, generated_vae, (mu, logvar, s) = G_curr(real_zero, opt.Noise_Amps, mode="rec")

        if opt.vae_levels >= opt.scale_idx + 1:
            rec_vae_loss = opt.rec_loss(generated, real) + opt.rec_loss(generated_vae, real_zero)
            kl_loss = kl_criterion(mu, logvar)
            entropy_loss = (s * torch.log(s + sys.float_info.epsilon)).mean()
            vae_loss = opt.rec_weight * rec_vae_loss + opt.kl_weight * kl_loss + opt.entropy_weight * entropy_loss
        ...
        if opt.visualize:
            ...
            if opt.vae_levels >= opt.scale_idx + 1:
                ...
                opt.summary.add_scalar('Video/Scale {}/Entropy'.format(opt.scale_idx), entropy_loss.item(), iteration)
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--entropy-weight', type=float, default=1., help='entropy loss weight')
